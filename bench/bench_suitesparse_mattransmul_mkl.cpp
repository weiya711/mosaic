...
static void bench_suitesparse_mattransmul_mkl(benchmark::State& state, bool gen=true, int fill_value=0) {
 ...
 //   std::cout << otherVec << std::endl;
   ...
   for (auto _ : state) {
    ...
    stmt = stmt.accelerate(new MklVadd(), accelerateExpr, true);
...
TACO_BENCH_ARGS(bench_suitesparse_mattransmul_mkl, mat_mattransmul, false)->UseRealTime();
